...
#if 0
static inline void pwm_set_mode(int id, int mode, int phase){
}
...
static inline void config_adc_channel1 (int chn_mic) {
	...
	if (chn_mic) {
		...
		analog_write (raga_pga_gain1, 0x33);
	}
	else {
...
static inline void config_adc (int chn_mic, int chn_adc, int sample) {
	...
	else if (sample == SYS_32M_AMIC_8K) { //32M / (77 + 11 * 16 * 2) = 72K
		reg_dfifo_scale = 7;				// down scale by 6
		reg_adc_period_chn0 = 244;
		reg_adc_period_chn12 = 8;
	}
	else {	//16M 16K
